...
export function UserHome() {
  ...
  const fetchAnnouncementImage = async (id, imageId) => {
    try {
      if (!imageId) {
        throw new Error("Image ID is missing");
      }
      const response = await fetch(`https://theezfixapi.onrender.com/api/v1/announcements/${id}/images/${imageId}`);
      if (!response.ok) {
        ...
        if (response.ok) {
          ...
          const formattedData = await Promise.all(
            data.map(async (announcement) => {
              const imageUrl = await fetchAnnouncementImage(announcement.id, announcement.imageId);
              return {
                ...announcement,
                imageUrl, // Add the dynamically fetched image URL
              };
            })
          );
          ...
          <SwiperSlide key={announcement.id}>
            <div className="p-6 bg-white shadow-xl rounded-xl hover:shadow-2xl transition-shadow duration-300">
              ...
              <img
                ...
                className="h-auto w-full object-cover rounded-lg cursor-pointer transition-transform duration-500 ease-in-out hover:scale-110"
